...
def detectMotion (pir_pin): 
    if io.input(pir_pin):
        ...
        if (motionDetected == True):
            print("PIR ALARM!")
            pirValue = io.input(pir_pin)
            motionDetected = False;
            soundAlarm()
            capturePicture()
            #send an email with date/time and picture attached
            sendEmail()
        time.sleep(10)
...
def capturePicture():
    ...
    camera.capture('PI-Secure.jpg')
    picture = 'PI-Secure.jpg'
    time.sleep(0.5)
...
def sendEmail(str1, str2):
    ...
    msg['Subject'] = "Pi Secure detected an intruder at " + today + ". a picture has been provided: " + picture
    msg['From'] = gmail_user
    msg['To'] = to
    time.sleep(5)
